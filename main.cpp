...
int values[5][3]={{882507206,0,0},{882504557,0,0},{882507214,0,0},{0,0,0},{0,0,0}};
...
void sendMessage() {
  ...
  int humidity_i=humidity*10;
  float temperature = dht.readTemperature();
  int temperature_i=temperature*10;
  
for (int i=0;i<5;i++){
  if(values[i][0]==node_id){
    values[i][1]=temperature_i;
    values[i][2]=humidity_i;
    }
}

Serial.printf("temp: %d, humid: %d, id:%d\n",temperature_i, humidity_i,node_id);
...
void receivedCallback(uint32_t from, String & msg) {
  ...
  Serial.printf("testing receiving id:%d temp: %d humid: %d \n", id_, temperature_, humidity_);
}
